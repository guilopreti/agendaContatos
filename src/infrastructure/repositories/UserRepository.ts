...
export class UserRepository implements IUserRepository {
  ...
  async findAll(): Promise<UserDTO[] | null> {
    try {
      const [result]: any = await this.pool.execute("SELECT * FROM users");
  ...
  async save(user: User): Promise<User> {
    try {
      ...
      console.log(
        `Erro ao persistir o registro (${user.email}) no banco: ${error}`,
      );
      throw new Error(
        `Erro ao persistir o registro (${user.email}) no banco: ${error}`,
      );
  ...
  async findByEmail(email: string): Promise<User | null> {
    try {
      ...
      throw new Error(
        `Erro ao recuperar o registro (${email}) no banco: ${error}`,
      );
    }
  }
